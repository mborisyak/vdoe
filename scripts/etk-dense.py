...
def train(seed: int, dataset: str, output: str, epochs: int, batch: int, log: str | None=None, progress: bool=True):
  ...
  *_, n_t = timestamps.shape
  ...
  timestamps = jnp.reshape(timestamps, shape=(total, n_exp, n_t))
  samples = jnp.reshape(samples, shape=(total, n_exp, n_t))

  print(f'Initials: {initials.shape}')
  print(f'timestamps: {timestamps.shape}')
  print(f'Samples: {samples.shape}')

  inference = NeuralProcess(n_input=n_t + n_init, n_output=n_t, n_latent=LATENT_DIM, rngs=rngs)
  ...
  def loss_fn(key, model, init, ts, ys):
    xs = pack(init, ts)
  ...
  def step(key: jax.Array, model, init, ts, ys, opt):
    loss, grad = nnx.value_and_grad(loss_fn, argnums=1)(key, model, init, ts, ys)
    opt.update(grad)
    return loss
...
def test(seed: int, dataset: str, model: str, log: str, batch: int, progress: bool=True):
  ...
  *_, n_t = timestamps.shape
  ...
  timestamps = jnp.reshape(timestamps, shape=(n_total, n_exp, n_t))
  samples = jnp.reshape(samples, shape=(n_total, n_exp, n_t))
  ...
  mu_inv_sigma_sqr, sum_inv_sigmas_sqr = inference.aggregate(mus, sigmas_raw, axes=(), keepdims=True, mask=None)
  mu_inv_sigma_sqr = jnp.concatenate([
    jnp.zeros(shape=(n_total, 1, mu_inv_sigma_sqr.shape[-1])),
    jnp.cumsum(mu_inv_sigma_sqr, axis=1)
  ], axis=1)
  sum_inv_sigmas_sqr = jnp.concatenate([
    jnp.ones(shape=(n_total, 1, sum_inv_sigmas_sqr.shape[-1])),
  ...
  *_, n_z = mu_posterior.shape
  n_samples = 64
  eps = jax.random.normal(rngs(), shape=(n_total, n_train + 1, n_samples, n_z))

  ### (*, n_e, n_samples, n_z)
  latent = eps * sigma_posterior[..., None, :] + mu_posterior[..., None, :]
  print(jnp.std(latent))

  ### (*, n_train, n_test, n_samples, n_z + n_x)
  Xz = jnp.concatenate([
    jnp.broadcast_to(X_test[:, None, :, None, :], (n_total, n_train + 1, n_test, n_samples, X_test.shape[-1])),
    jnp.broadcast_to(latent[:, :, None, :, :], (n_total, n_train + 1, n_test, n_samples, latent.shape[-1])),
  ], axis=-1)
  ...
  for j in range(n_train + 1):
    if j > 0:
      ts = timestamps[display_index, j - 1, :]
      ys = samples[display_index, j - 1, :]

      axes[j, 0].scatter(ts, ys, color=plt.cm.tab10(1))

    for i in range(n_test):
      ts = timestamps[display_index, n_train + i, :]
      ys = samples[display_index, n_train + i, :]

      pred = predictions[display_index, j, i, :, :]
      low = np.quantile(pred, q=0.1, axis=0)
      med = np.median(pred, axis=0)
      high = np.quantile(pred, q=0.9, axis=0)
